# Spider Swing — collaboration model
...
- When a doc and a source file disagree: Source code, executable tests, merged commits, and build artifacts outrank stale prose. When documentation disagrees with verified source, follow the source and correct the documentation in the same PR. Record genuine design changes in the decision ledger rather than preserving contradictory history in binding docs.
- Staleness review cadence: Living ledgers (docs/current-state.md, docs/CAPABILITIES.md, control/status.md) are re-verified against the live surface every session that changes behaviour, and at minimum every 14 days per the configured staleness cadence. Binding docs and ADRs are reviewed whenever a merged change contradicts them, and the contradiction is corrected in the same PR rather than deferred.
